  ...
  <div class="main">
    <div class="info">
      <a-tooltip placement="bottom" >
        <template slot="title">
            <span>本工具用于在文件名前后添加文字，例如：考研数学.mp4 -> 考研数学(张雪峰).mp4</span>
        </template>
    ...
    <div>
      <a-radio-group @change="whereindexChange" v-model="whereindex">
        <a-radio :style="radioStyle" :value="1">前方插入</a-radio>
        <a-radio :style="radioStyle" :value="2">后方插入</a-radio>
    </a-radio-group>
    </div>
    <div class="list">
  ...
  methods: {
    ...
    preview(){
      ...
      for (let f of this.fileList) {
        ...
        let oldname_noext = f.name.replace(ext, '')

        let newname = this.whereindex==1?`${this.extraStr}${oldname}`:`${oldname_noext}${this.extraStr}${ext}`;
